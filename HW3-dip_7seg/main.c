...
unsigned int num[17] = {Number_0, Number_1, Number_2, Number_3, Number_4, Number_5, Number_6, Number_7,
...
// strings for student id
const char *stu[] = {"40626251", "40626216"};
const int siz = 8;
enum {
	N_ROY=0,
	N_HW
...
//
int lim = 0; // common counter
// show the first std id from right to left
void show_roy()
{
	int i; // idx of stu   [0, lim]
	int j; // idx of frame [7-lim, 7]
	clean();
	for(i = 0, j = 7-lim;
		i <= lim && j <= 7;
		i++, j++)
		frame[j] = stu[N_ROY][i]-'0';
	//
	if(lim < siz-1)
		lim++;
}
// show the second std id from left to right
void show_halloworld()
{
	int i; // idx of stu   [7-lim, 7]
	int j; // idx of frame [0, lim]
	clean();
	for(i = 7-lim, j = 0;
		i <= 7 && j <= lim;
		i++, j++)
		frame[j] = stu[N_HW][i]-'0';
	//
	if(lim < siz-1)
		lim++;
}
